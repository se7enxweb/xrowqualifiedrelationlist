...
{else}	{* Standard mode is browsing *}
        ...
        <tr class="bglight">
            ...
            <th>{'Qualifier'|i18n( 'design/standard/content/datatype' )}</th>
        ...
        {* Qualify relation. *}
        <td>
        <select name="{$attribute_base}_qualifier_{$attribute.id}[]">

        {foreach ezini( 'Settings', 'QualifierList','xrowqualifiedrelationlist.ini' ) as $key => $name }
        <option value="{$key}" {if or($key|eq($:item.qualifier),and($:item.qualifier|eq(''),$key|eq(ezini( 'Settings', 'Default','xrowqualifiedrelationlist.ini' ))))} selected="selected" {/if}>{$name|wash}</option>
        {/foreach}
        </select>
